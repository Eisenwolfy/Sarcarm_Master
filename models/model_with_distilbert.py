...
SUBSAMPLE_FRAC = 1.0  # set e.g. 0.2 to use only 20% of the data while iterating fast
MAX_LEN_COMMENT = 40
MAX_LEN_PARENT = 40
LSTM_HIDDEN_DIM = 64
...
def clean_text(text):
    """
    Only remove URLs and collapse whitespace. Unlike the from-scratch LSTM
    version, we do NOT strip punctuation or lowercase aggressively here:
    DistilBERT's own tokenizer (WordPiece) is trained on natural text and
    already handles casing/punctuation well, so over-cleaning would throw
...
class Attention(nn.Module):
    ...
    def forward(self, lstm_out, mask):
        ...
        # of a hardcoded -1e9. Under autocast (mixed precision), scores are
        # float16, and float16's range tops out around +/-65504 -- -1e9
        # overflows it and crashes. torch.finfo(dtype).min is always safe.
...
class SarcasmBertLSTM(nn.Module):
    def __init__(self, lstm_hidden_dim, freeze_layers=4):
        ...
        # Sharing keeps parameter count reasonable and both branches use the
        # same "understanding" of English.
        self.bert = DistilBertModel.from_pretrained("distilbert-base-uncased")
        bert_dim = self.bert.config.hidden_size

        # Freeze the embedding layer and the first N transformer blocks.
        # DistilBERT has 6 transformer layers total; freezing the earlier
        # (more general) ones and only fine-tuning the later (more
        # task-specific) ones is a common trick to save memory/compute and
        # reduce overfitting on a dataset this size.
...
# ---------------------------------------------------------------------
# BERT weights are pretrained and only need small updates (2e-5 is the
# standard fine-tuning LR). The new LSTM/attention/classifier layers are
